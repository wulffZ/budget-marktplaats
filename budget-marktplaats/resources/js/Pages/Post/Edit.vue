...
// Set photos as null whenever refreshed.
let currentPhotos = ref([]);
let toBeDeleted = [];
let toBeAdded = ref([]);


// Set the current photos in the request as selected value.
for (let i = 0; i < props.post.images.length; i++) {
    currentPhotos.value.push(props.post.images[i]);
}
...
function addPhoto() {
    toBeAdded.value.push(document.getElementById('photos').files);
}

function removePhoto(index) {
    toBeDeleted.push(currentPhotos.value[index]);
                    ...
                    <form @submit.prevent="submit">
                        ...
                        <div class="mb-6">
                            <label for="Description"
                                   class="block mb-2 text-sm font-medium text-white">Beschrijving</label>
                            <textarea
                                type="text"
                        ...
                        <div class="mb-6">
                            ...
                            <select name="category" v-model="form.category"
                                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-secondary focus:border-secondary block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                        ...
                        <label class="block mb-2 text-sm font-medium text-white">Huidige geuploade
                            photos</label>
                        <div class="grid grid-cols-4 max-w-6xl mx-auto mb-6">
                            <div v-for="(photo, index) in currentPhotos" :key="photo['id']">
                                <img :src="`/storage/photos/${ photo['file_uri']}`" v-if="photo['file_uri']"
                                     class="w-48 rounded-md hover:opacity-50" @click="removePhoto(index)">
                            </div>
                        </div>
                        <div class="mb-6">
                            <label for="Photos" class="block mb-2 text-sm font-medium text-white">Voeg foto's toe.</label>
                            <input id="photos" type="file" v-on:change="addPhoto" class="form-control" name="photos"
                                   multiple/>
                            <div v-if="form.errors.currentPhotos" class="text-sm text-red-600">
                                {{ form.errors.currentPhotos }}
                            </div>
                        </div>
                        <button
                            type="submit"
